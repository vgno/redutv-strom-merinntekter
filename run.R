...
national %>%
  filter(param_price_baseline == 0.4, param_prop_spot_eksponert == .7) %>%
  group_by(as.Date(date))

weekly %>%
  filter(param_price_baseline == 0.4, param_prop_spot_eksponert == .7) %>%
  ggplot(aes(x=date, y=en.staten.spot.cum/1e9)) +
  geom_line()
...
# minimal %>% jsonlite::toJSON() %>% write_file("~/Sites/spesial/merinntekt.json")
